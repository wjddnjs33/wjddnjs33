# Introduce

```
Hi there, My name is Jeongwon Jo(Pocas). I'm a newbie studying about web security/development and english in Seoul, Republic of Korea. And I sometimes do CTF or Bug Hunting. And Recently I am studying development. Web development uses only node.js, flask, django web frameworks to develop back-end.
```

---
# Just reported things.

```
HackerOne Meredith #1224657 Open Redirect to Reflected XSS Resolve (Jeongwon Jo)
REDACTED REDACTED #13166 Stored XSS Duplicates Fucking (Jeongwon Jo)
REDACTED National University University #None Reflected XSS Resolve (Jeongwon Jo)
HackerOne RMS #None Reflected XSS Resolve (Jeongwon Jo)
```

---
